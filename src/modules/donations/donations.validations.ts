...
export const updateDonationStatusSchema = z
  .object({
    ...
    receivedQuantity: z.coerce.number().int().min(1).optional(),
    receptionNotes: z
      .string()
      .trim()
      .max(2000, VALIDATION_MESSAGES.DONATION_NOTES_MAX_LENGTH)
  ...
  .superRefine((data, ctx) => {
    if (data.status === DonationStatus.RECEIVED && data.receivedQuantity === undefined) {
      return;
    }
    if (data.status !== DonationStatus.RECEIVED && data.receivedQuantity !== undefined) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: VALIDATION_MESSAGES.DONATION_RECEIVED_QUANTITY_ONLY_ON_RECEIVE,
